...
describe("Mapathon Report Form Component", () => {
  ...
  it('allows form input changes', async () => {
    ...
    // default form values
    expect(startDate.value).toContain(format(addHours(new Date(), -1), "d MMMM, yyyy h:mm aa")); //start date datepicker
    ...
    // error
    expect(getByText(/It was not possible to run this query/i)).toBeInTheDocument();
