...
test('Reports View', () => {
  ...
  expect(getByText(/Know how specific mappers have contributed to OSM/i)).toBeInTheDocument();
  expect(getByText("Organisation Reports")).toBeInTheDocument();
  expect(getByText("Organisation Reports").closest('a').href).toContain('/organisation-report');
  expect(getByText("Organisation Report")).toBeInTheDocument();
  expect(getByText(/Country Reports/i)).toBeInTheDocument();
  expect(getByText(/Country Reports/i).closest('a').href).toContain('/country-report');
  expect(getByText(/Learn about the growth of OSM features in a country from the beginning of time on a monthly basis/i)).toBeInTheDocument();
});
